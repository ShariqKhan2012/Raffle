...
export function RaffleCard() {
  ...
  const { data, refetch, isLoading } = useReadContracts({
    contracts: raffleAddress
      ? [
        ...
        { address: raffleAddress, abi: RAFFLE_ABI, functionName: "getRaffleCounter" },   // 11
        { address: raffleAddress, abi: RAFFLE_ABI, functionName: "getRaffleHistory" },   // 12
      ]
      : [],
  ...
  const raffleCounter = data?.[11]?.result as bigint | undefined;
  const raffleHistory = data?.[12]?.result as
    | readonly {
      prizeMoney: bigint;
      ...
      <div className="mx-auto  space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            />
            Raffle # {raffleCounter?.toString() ?? "—"} : {isProcessing ? "Drawing Winner…" : "Open — Enter Now"}
          </span>
